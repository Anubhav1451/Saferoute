...
os.environ['DATABASE_URL'] = 'sqlite:///./test_fresh.db'
...
def main():
    ...
    orphan_nodes = session.query(GraphNode).filter(
        ~Ghost.node.id.in_(session.query(GraphEdge.source_node_id)) &
        ~Ghost.node.id.in_(session.query(GraphEdge.dest_node_id))
    ).count() if False else 0  # placeholder

    # Actually compute orphan nodes correctly:
