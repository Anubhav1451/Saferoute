...
def test_caching_and_safety():
    """Test that the second request for the same corridor uses the cache and that safest route is safer."""
    ...
    # We'll create some mock safety data for the corridor with varying safety scores
    import math
    ...
    # Generate 10 safety nodes along the corridor
    safety_points = generate_points(28.6315, 77.2167, 29.9660, 77.5540, 10)
    mock_safety_nodes = []
    for i, (lat, lon) in enumerate(safety_points):
        # Safety score increases linearly from 0.5 at start to 0.9 at end
        safety_score = 0.5 + 0.4 * (i / 10.0)
        # Alternate lighting and crowd to add some risk variation
        lighting = LightingLevel.LOW if i % 2 == 0 else LightingLevel.MEDIUM
        crowd = CrowdDensity.SPARSE if i % 3 == 0 else CrowdDensity.NORMAL
        node = create_mock_safety_node(lat, lon, safety_score=safety_score, lighting_level=lighting, crowd_density=crowd)
        mock_safety_nodes.append(node)

    # Create a few mock crime hotspots (place some near the middle)
    mock_crime_hotspots = [
    ...
    def mock_get_nearby_safety_data_bounding_box(min_lat, max_lat, min_lon, max_lon):
        ...
        return (nodes, hotspots, reports, mock_segment_risks)  # Added empty list for segment_risks
    ...
    with patch('app.services.routing.SafetyRoutingService.get_nearby_safety_data_bounding_box') as mock_bounding_box, \
        ...
        print(f"Safest distance: {result1['safest_distance']:.2f} meters")
        print(f"Fastest distance: {result1['fastest_distance']:.2f} meters")
        ...
        print(f"Safest distance: {result2['safest_distance']:.2f} meters")
        print(f"Fastest distance: {result2['fastest_distance']:.2f} meters")
        ...
        # Check if safest route is safer than fastest route (or at least not less safe)
        safest_score = result1['safest_safety_score']
        fastest_score = result1['fastest_safety_score']
        if safest_score >= fastest_score:
            print("\nSUCCESS: Safest route is safer than or equal to fastest route.")
        else:
            print(f"\nWARNING: Safest route is less safe than fastest route by {fastest_score - safest_score:.4f}")
