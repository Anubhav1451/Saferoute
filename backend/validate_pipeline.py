...
def main():
    ...
    # nodes not referenced as source or dest in any edge
    orphan_nodes = session.query(GraphNode).filter(
        ~GraphNode.id.in_(session.query(GraphEdge.source_node_id)) &
        ~GraphNode.id.in_(session.query(GraphEdge.dest_node_id))
    ).count()

    # Orphan edges (dangling references) - should be zero due to FK but check
